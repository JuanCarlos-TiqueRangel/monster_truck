...
from utils.svgp_retrain_manager import SVGPRetrainManager
...
class MPPICarControllerNode(Node):
    def __init__(self, cfg: MPPIConfig):
        ...
        self.gp_pose_x: SVGPManager = SVGPManager.load(self.cfg.gp_pos_x_path, device=self.device)
        self.gp_vx: SVGPManager     = SVGPManager.load(self.cfg.gp_vx_path, device=self.device)
        ...
        # SVGP warm-update retrainer
        self.retrain = SVGPRetrainManager(
            cfg=self.cfg,
            device=self.device,
            model_lock=self.model_lock,
            logger=self.get_logger(),
