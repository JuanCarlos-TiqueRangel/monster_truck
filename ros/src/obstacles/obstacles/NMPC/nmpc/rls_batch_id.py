...
It REUSES rls_training.py's excitation schedule (identical maneuvers) and reads MuJoCo's
exact acceleration (qacc) as the target. Output: a weights+/-sigma table, train-vs-CV RMSE,
rls_batch.npz, and a parity/residual figure.
...
# ============================================================
# 1) Data collection -- run the SAME excitation, log matched (phi, qacc) pairs
# ============================================================

def collect_dataset(p: WheelieParams):
    model = mujoco.MjModel.from_xml_path(str(RT.XML_PATH))
    data = mujoco.MjData(model)
    xq, xv = RT.get_joint_addresses(model, "root_x")
    zq, _ = RT.get_joint_addresses(model, "root_z")
    pq, pv = RT.get_joint_addresses(model, "root_pitch")
    drive = RT.get_actuator_id(model, "drive_motor")
    cmin = float(model.actuator_ctrlrange[drive, 0])
    cmax = float(model.actuator_ctrlrange[drive, 1])
    sim_dt = float(model.opt.timestep)
    ctrl_steps = max(1, int(round(RT.CTRL_DT / sim_dt)))

    def read_state():
        return np.array([float(data.qpos[xq]), float(data.qvel[xv]),
                         RT.PITCH_SIGN * float(data.qpos[pq]),
                         RT.PITCH_SIGN * float(data.qvel[pv])], dtype=float)

    def reset_pose():
        data.qpos[:] = 0.0
        data.qvel[:] = 0.0
        data.qpos[zq] = RT.INITIAL_Z
        data.ctrl[drive] = 0.0
        mujoco.mj_forward(model, data)

    rows = []
    reset_pose()
    for (name, dur, fn, reset_after) in RT.build_schedule(p):
        t0 = data.time
        for s in range(int(round(dur / sim_dt))):
            if s % ctrl_steps == 0:
                st = read_state()
                tau = float(np.clip(fn(data.time - t0, st), p.tau_min, p.tau_max))
                data.ctrl[drive] = float(np.clip(RT.ACTUATOR_SIGN * RT.TAU_TO_CTRL * tau, cmin, cmax))
                # qacc consistent with (state, tau) -> matched target for phi (same instant)
                mujoco.mj_forward(model, data)
                v_dot = float(data.qacc[xv])
                omega_dot = RT.PITCH_SIGN * float(data.qacc[pv])
                x, v, th, om = st
...
def main():
    ...
    Phi_v, Phi_w, y_v, y_w = collect_dataset(p)
