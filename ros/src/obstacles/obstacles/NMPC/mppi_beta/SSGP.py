...
class SSGPResidual:
    ...
    def _init(self):
        ...
        for item in self._buf:
            z, rv, rw = item[0], item[1], item[2]  # Ignore r_obstacle (item[3])
    ...
    def observe(self, z, r_v, r_omega, r_obstacle=None):
        """Observe residuals: velocity, angular velocity, and optional obstacle distance."""
        self.n_seen += 1
        z = np.asarray(z, float)
        if not self.ready:
            self._buf.append((z, float(r_v), float(r_omega), float(r_obstacle) if r_obstacle is not None else 0.0))
            if len(self._buf) >= self.warmup:
                self._init()
            return
        self.gp_v.update(z, r_v)
        self.gp_w.update(z, r_omega)
        # Note: r_obstacle is logged but not yet used in predictions (future extension)
        self._refresh_alpha()
    ...
    def save_checkpoint(self, filepath):
        """Save learned GP state for reuse in future episodes."""
        import pickle
        checkpoint = {
            'Z': self.Z,
            'l': self.l,
            ...
            'gp_v_m': self.gp_v.m.copy() if self.ready else None,
            'gp_w_m': self.gp_w.m.copy() if self.ready else None,
            'gp_v_S': self.gp_v.S.copy() if self.ready else None,
            'gp_w_S': self.gp_w.S.copy() if self.ready else None,
            'n_seen': self.n_seen,
        }
        with open(filepath, 'wb') as f:
            pickle.dump(checkpoint, f)
        print(f"✓ Saved GP checkpoint to {filepath} (n_seen={self.n_seen})")

    def load_checkpoint(self, filepath):
        """Load a previously learned GP: hyperparameters AND posterior (m, S).

        Carrying the posterior over is what makes the model genuinely persist --
        episode j+1 starts already knowing episode j's residual function, so the
        rollout is accurate from t=0 (no 60-step warmup). The kernel hyperparameters
        (Z, lengthscales) are fixed, so restoring (m, S) is exact, not overconfident.
        """
        import pickle
        try:
            ...
            # Restore kernel hyperparameters
            self.Z = checkpoint['Z']
            self.l = checkpoint['l']
            self.sf2 = checkpoint['sf2']
            self.sn2 = checkpoint['sn2']
            self.active = np.ones(self.Z.shape[0], dtype=bool)

            # Rebuild the GPs on the saved kernel, then restore the LEARNED posterior
            # (m, S) so the residual function is carried across episodes.
            self.gp_v = StreamingSparseVGP(self.Z, self.l, self.sf2, self.sn2, q=self.q)
            self.gp_w = StreamingSparseVGP(self.Z, self.l, self.sf2, self.sn2, q=self.q)
            if checkpoint.get('gp_v_m') is not None:
                self.gp_v.m = np.asarray(checkpoint['gp_v_m']).copy()
                self.gp_v.S = np.asarray(checkpoint['gp_v_S']).copy()
                self.gp_w.m = np.asarray(checkpoint['gp_w_m']).copy()
                self.gp_w.S = np.asarray(checkpoint['gp_w_S']).copy()
            self.ready = True

            # n_seen carries over so refit cadence and diagnostics stay meaningful;
            # the buffer stays empty (we're past warmup) and online updates continue.
            self.n_seen = int(checkpoint.get('n_seen', 0))
            self._refresh_alpha()
            print(f"✓ Loaded GP (hyperparams + posterior) from {filepath} "
                  f"(n_seen={self.n_seen}, ‖m_v‖={np.linalg.norm(self.gp_v.m):.3f})")
            return True
        except FileNotFoundError:
            print(f"  No checkpoint found at {filepath} (will train from scratch)")
            return False
