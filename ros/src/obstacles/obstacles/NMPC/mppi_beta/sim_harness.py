...
def run_episode(controller, p, gp_cfg, rls_cfg, *, xml_path=XML_PATH,
                ...
                render=False, verbose=False, print_every=10, gp_checkpoint=None,
                ilc=None):
    """Run one closed-loop episode; return dict(history, metrics, gp, rls_b0).

    Args:
        gp_checkpoint: Optional path to saved GP state. If provided, loads checkpoint
                      (hyperparams + posterior) instead of training from scratch.
        ilc:          Optional ILC object. If provided, its path-indexed feedforward
                      torque ilc.feedforward(x) is added to the MPPI command each
                      step. ILC is the across-episode memory that makes the truck
                      reach the goal faster lap over lap.
    """
    ...
    # Load previous learned state if checkpoint provided (skip 60-step warmup!)
    if gp_checkpoint is not None:
        from pathlib import Path
        checkpoint_path = Path(gp_checkpoint)
        if checkpoint_path.exists():
            gp.load_checkpoint(str(checkpoint_path))
    ...
    def do_control():
        ...
        r_v = r_w = r_obs = gp_v = gp_w = gp_obs = 0.0
        gp_v_std = gp_w_std = gp_obs_std = float(np.sqrt(getattr(gp_cfg, "sf2", 1.0)))
        if last_state is not None:
            ...
            # REMOVED: Obstacle position learning (not generalizable)
            # Instead: focus on core residuals which are more fundamental
        ...
        # ILC feedforward (across-episode memory): add the learned, path-indexed
        # torque on top of MPPI's feedback, then clip. tau_cmd carries the TOTAL
        # applied torque so the GP learns the residual of what was actually applied
        # and MPPI's next-step smoothness sees the real previous actuation.
        if ilc is not None:
            tau = tau + ilc.feedforward(x)
    ...
    if render:
        ...
        print("\n" + "="*70)
        print("🎬 LAUNCHING MUJOCO VIEWER (Real-time simulation)")
        print("="*70)
        print("Window should pop up now. If not visible:")
        print("  - Check taskbar/window list for MuJoCo")
        print("  - Alt+Tab to switch windows")
        print("  - Check secondary displays")
        print("="*70 + "\n")
        ...
        with mj_viewer.launch_passive(model, data) as viewer:
            ...
            while viewer.is_running() and data.time < sim_time:
                ...
                # End episode immediately when goal is reached
                if len(hist["x"]) > 0 and hist["x"][-1] >= goal_x:
                    break
        print("✓ Viewer closed\n")
    else:
        print("\n⚡ Running in HEADLESS mode (fast, no viewer)\n")
    ...
    # rls_b0 = dv_dot/dtau of the learned model; ILC scales its step by it.
    return {"history": hist, "metrics": metrics, "gp": gp,
            "rls_b0": float(rls_v.theta[0])}
