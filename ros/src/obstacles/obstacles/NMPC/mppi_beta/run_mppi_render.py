...
With persistent GP learning: GP state is saved after each run and loaded on
next run, so learning accumulates across episodes (no 60-step warmup each time!).
"""
...
def main():
    ...
    # Setup persistent GP learning
    gp_checkpoint_file = Path(__file__).with_name("gp_learned_checkpoint.pkl")

    # Display checkpoint status
    if gp_checkpoint_file.exists():
        print(f"\n{'='*70}")
        print("PERSISTENT GP LEARNING: Will load previous knowledge (skip warmup!)")
        print(f"{'='*70}\n")
    else:
        print(f"\n{'='*70}")
        print("First run: training GP from scratch (60-step warmup)")
        print(f"{'='*70}\n")

    # Create controller
    ctrl = WheelieMPPI(PARAMS, MPPI_CFG, GP_CFG)

    # Run episode with optional checkpoint loading
    out = run_episode(ctrl, PARAMS, GP_CFG, RLS_CFG,
                      sim_time=SIM_TIME, render=RENDER, verbose=True,
                      gp_checkpoint=str(gp_checkpoint_file))

    print("\nMetrics:")
    for k, val in out["metrics"].items():
        print(f"  {k:16s}: {val}")

    # Save GP checkpoint for next episode
    print("\n" + "="*70)
    print("PERSISTENT GP LEARNING: Saving learned state...")
    gp = out["gp"]
    gp.save_checkpoint(str(gp_checkpoint_file))
    print("="*70)
    ...
    data_file = Path(__file__).with_name("mppi_beta_episode_data.json")
    with open(data_file, 'w') as f:
        json.dump(data, f, indent=2)
    print(f"\n✓ Data saved to {data_file.name}")
    print(f"  Now run: python3 make_paper_plot.py")
    print(f"\n✓ Next run will load learned GP (no 60-step warmup!)")
