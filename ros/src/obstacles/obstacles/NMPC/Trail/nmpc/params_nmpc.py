...
class MPCConfig:
    ...
    # MBRL REWARD: maximise forward PROGRESS (= minimise time). The planner optimises this return
    # against the LEARNED GP model, so -- because the model knows rearing reduces the obstacle
    # blockage -- the wheelie EMERGES where it pays off, with no hand-set angle/location.
    w_progress: float = 15.0

    # Smooth flip penalty: q_flip barriers pitch beyond theta_soft_deg (prevents tip-over)
    # WITHOUT forbidding the wheelie needed to climb.
    q_flip: float = 2000.0
