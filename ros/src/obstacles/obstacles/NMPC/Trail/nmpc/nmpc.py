...
# def casadi_matern32_mean(z, Z, alpha, x_mean, x_std, ell, sf2, y_mean):
#     """Symbolic ARD Matern-3/2 sparse-GP mean, matching online_sparse_gp.OnlineSparseGP:
#         zc    = (z - x_mean) / x_std                      # standardize (the model fits here)
#         mu(z) = y_mean + sum_i alpha_i * sf2 (1+c_i) exp(-c_i),   c_i = sqrt(3) ||(zc - Z_i)/ell||
#     Z are the STANDARDIZED inducing points and alpha = Kzz^{-1} m_u the dual weights, both
#     supplied each solve by OnlineGPResidual.mpc_params(). x_mean/x_std/ell are d-vectors,
#     sf2/y_mean scalars -- all runtime parameters (the GP fits its own hyperparameters)."""
#     M = Z.shape[0]
#     zc = (z - x_mean) / x_std
#     out = y_mean
#     for i in range(M):
#         diff = (zc - Z[i, :].T) / ell
#         r = ca.sqrt(ca.dot(diff, diff) + 1e-9)            # +eps: smooth the Matern sqrt kink for IPOPT
#         c = ca.sqrt(3.0) * r
#         out += alpha[i] * (sf2 * (1.0 + c) * ca.exp(-c))
#     return out
...
#         self.d = gp_cfg.n_features                  # GP feature dim (= 2: x, theta)
...
#         # Both residual channels drive the rollout. The omega_dot channel stays usable because the
#         # per-episode BATCH fit() estimates its noise variance, so the impulsive +-tens rad/s^2
#         # contact spikes are absorbed as noise and gp_omega_dot stays smooth (no clip, no bounds).
#         gp_v_dot = casadi_matern32_mean(z, Z, alpha_v_dot, x_mean, x_std, ell_v, sf2_v, ymean_v)
#         gp_omega_dot = casadi_matern32_mean(z, Z, alpha_omega_dot, x_mean, x_std, ell_w, sf2_w, ymean_w)
...
#         # Parameter vector. The GP block is OnlineGPResidual.mpc_params() (Matern-3/2):
#         #   state(4) ref(4) tau_prev(1) a_rls(n_rls=10)
#         #   | Z(M*d) alpha_v(M) alpha_w(M) x_mean(d) x_std(d) ell_v(d) ell_w(d) sf2_v sf2_w ymean_v ymean_w
#         n_base = 9 + self.n_rls
#         gp_size = M * d + 2 * M + 4 * d + 4
