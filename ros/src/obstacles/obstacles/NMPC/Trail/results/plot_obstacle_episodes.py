...
def obstacle_xs(xml_path):
    """Read obstacle geom x-positions STRAIGHT from the MuJoCo XML so the markers match
    the actual scene (commented-out geoms are ignored by the parser). Picks geoms whose
    name starts with 'obs_box' or 'pole_box'."""
    try:
...
def main():
    ...
    for xo in obstacle_xs(here.parent / "monster_truck_flip_2d.xml"):
