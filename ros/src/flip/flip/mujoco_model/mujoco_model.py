...
class MujocoImuNode(Node):
    def __init__(self):
        ...
        self.ctrl_dt = 0.01  # 100 Hz controller / publish loop
        ...
        self.u_esc = 0.0  # filtered ESC command
        ...
        # Input / low-speed logic
        self.neutral_deadband = 0.02
        self.brake_speed_thresh = 8.0  # wheel rad/s threshold

        # Passive driveline / wheel-side drag
        ...
        # Warn if XML ctrlrange is smaller than what Python may command
        try:
    ...
    # ------------------------------------------------------------
    # Drive model 1: improved simple envelope
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # Drive model 2: electrical ESC + battery + motor approximation
    # ------------------------------------------------------------
    ...
    def _compute_drive_torque_from_throttle(self, u: float) -> float:
        ...
        elif self.drive_model == "electrical":
    ...
    def timer_cb(self) -> None:
        ...
        # 3) throttle -> physical driveline torque
        tau_cmd = self._compute_drive_torque_from_throttle(u_eff)

        # 4) apply actuator command
        self.data.ctrl[:] = 0.0
