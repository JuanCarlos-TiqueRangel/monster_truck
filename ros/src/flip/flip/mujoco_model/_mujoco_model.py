...
class MujocoImuNode(Node):
    def __init__(self):
        ...
        # ---------------- Command ----------------
        # /cmd_action is interpreted as normalized throttle in [-1, 1]
        self.last_action = 0.0

        # ---------------- 4S motor / drivetrain approximation ----------------
        # These parameters define the throttle -> driveline torque map.
        self.V_nom = 14.8             # 4S nominal voltage
        self.Kv_rpm_per_V = 2800.0    # motor Kv
        self.final_drive = 18.76      # final drive ratio
        self.eta = 0.85               # driveline efficiency approximation
        self.I_limit = 130.0          # ESC current class
        self.neutral_deadband = 0.02  # small neutral deadband

        # Kt from Kv
        self.Kt = 60.0 / (2.0 * np.pi * self.Kv_rpm_per_V)  # N*m/A

        # Motor-side torque limit approximation
        self.tau_motor_max = self.Kt * self.I_limit

        # Total driveline output torque limit approximation
        self.tau_drive_max = self.eta * self.final_drive * self.tau_motor_max

        # No-load wheel speed approximation on 4S
        omega_motor_nl = self.Kv_rpm_per_V * self.V_nom * 2.0 * np.pi / 60.0
        self.omega_wheel_nl = omega_motor_nl / self.final_drive

        self.get_logger().info(
            f"Drive model ready: tau_drive_max={self.tau_drive_max:.3f} N*m, "
    ...
    # ------------------------------------------------------------
    # Control mapping: throttle -> single driveline torque
    # ------------------------------------------------------------
    def _compute_drive_torque_from_throttle(self, u: float) -> float:
        """
        u in [-1, 1] is normalized throttle.
        Returns one driveline torque command for the single MuJoCo actuator.
        """

        # small neutral deadband
        ...
        # One motor -> use average wheel speed as a simple driveline speed proxy
        w_avg = 0.25 * (w_fl + w_fr + w_rl + w_rr)

        # Simple speed-dependent torque envelope:
        # torque reduces as wheel speed approaches no-load speed
        speed_factor = max(0.0, 1.0 - abs(w_avg) / max(self.omega_wheel_nl, 1e-6))

        tau = u * self.tau_drive_max * speed_factor

        # Must remain within XML actuator ctrlrange
        return float(np.clip(tau, -7.0, 7.0))
    ...
    def _publish_pole_pose(self, stamp):
        ...
        p = self.data.geom_xpos[self.pole_geom_id]

        # World rotation matrix of the geom (flattened 9 values)
        xmat = self.data.geom_xmat[self.pole_geom_id]
    ...
    def timer_cb(self) -> None:
        # Convert normalized throttle to one driveline torque command
        tau_cmd = self._compute_drive_torque_from_throttle(self.last_action)
