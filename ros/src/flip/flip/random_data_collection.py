...
from utils.geometry import up_and_updot_from_quat_gyro
...
class Config:
    ...
    sample_dt: float = 0.02          # [s] 50 Hz logging + random input update
...
def setup_figure(cfg: Config):
    ...
    fig, (ax1, ax2, ax3) = plt.subplots(
        3, 1, figsize=(18, 10), sharex=True, constrained_layout=True
    )

    line_width = 2.5
    (line_up_z,) = ax1.plot([], [], lw=line_width, label="up_z")
    (line_up_z_dot,) = ax2.plot([], [], lw=line_width, label="up_z_dot")
    (line_u,) = ax3.plot([], [], lw=line_width, label="u")

    ax1.set_ylabel("up_z")
    ax1.grid(True)
    ax1.legend(loc="upper right")

    ax2.set_ylabel("up_z_dot")
    ax2.grid(True)
    ...
    return fig, (ax1, ax2, ax3), line_up_z, line_up_z_dot, line_u


def update_plot(node, fig, axes, line_up_z, line_up_z_dot, line_u):
    ax1, ax2, ax3 = axes
    ...
    up_z = np.asarray(node.up_z_log, dtype=float)
    up_z_dot = np.asarray(node.up_z_dot_log, dtype=float)
    u = np.asarray(node.u_log, dtype=float)

    line_up_z.set_data(t, up_z)
    line_up_z_dot.set_data(t, up_z_dot)
    line_u.set_data(t, u)
    ...
    if len(up_z) > 1:
        ymin, ymax = np.min(up_z), np.max(up_z)
        pad = max(0.05, 0.1 * max(1e-6, ymax - ymin))
        ax1.set_ylim(ymin - pad, ymax + pad)

    if len(up_z_dot) > 1:
        ymin, ymax = np.min(up_z_dot), np.max(up_z_dot)
        pad = max(0.05, 0.1 * max(1e-6, ymax - ymin))
...
class MujocoRandomCmdLogger(Node):
    ...
    def imu_cb(self, msg: Imu):
        ...
        self.up_z, self.up_z_dot, _ = up_and_updot_from_quat_gyro(
    ...
    def sample_timer_cb(self):
        ...
        self.up_z_log.append(float(self.up_z))
        self.up_z_dot_log.append(float(self.up_z_dot))
    ...
    def save_npz(self):
        ...
        print(f"Done. Samples: {len(self.up_z_log)}  wall time: {self.t_wall_log[-1]:.3f}s")
...
def main():
    ...
    fig, axes, line_up_z, line_up_z_dot, line_u = setup_figure(cfg)
    ...
    try:
        while rclpy.ok() and not node.stop_requested:
            ...
            if cfg.online_plot and (now - last_refresh_wall >= 1.0 / cfg.refresh_hz):
                update_plot(node, fig, axes, line_up_z, line_up_z_dot, line_u)
