...
# Viewer is OFF by default for faster runs.
# Enable with: MUJOCO_ENABLE_VIEWER=1 python ...
...
class MujocoImuNode(Node):
    def __init__(self, enable_viewer: bool = ENABLE_VIEWER):
        ...
        if self.enable_viewer:
            ...
            if mjviewer is None:
                self.get_logger().warning("Viewer requested but mujoco.viewer could not be loaded.")
            else:
                try:
                    self.viewer = mjviewer.launch_passive(self.model, self.data)
                except Exception as e:
                    self.get_logger().warning(f"Viewer not started: {e}")
        ...
        self.get_logger().info(
            f"MujocoImuNode ready. START_FLIPPED={START_FLIPPED}. enable_viewer={self.enable_viewer}. "
