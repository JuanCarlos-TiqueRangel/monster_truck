...
from rls import rls_dynamics_torch
...
class MPPITorch:
    ...
    def __init__(self, p, cfg, rls_parameters, device=None, fast=True, integrator="euler", dtype=None, live_plot=False):
    ...
    def _plot_setup(self, plt, ref, n_best):
        ...
        ax.set(xlabel="x [m]", ylabel="theta [deg]",
               xlim=(-2.0, goal_x + 2.0), ylim=(-200, 200),
    ...
    def set_rls_parameters(self, parameters):
        """Copy the latest CPU-side RLS estimate to the rollout device."""
        self.rls_parameters.copy_(
            torch.as_tensor(parameters, dtype=self.dtype, device=self.device)
        )

    # ---- learned RLS dynamics ---
    def _deriv(self, s, tau):
        return rls_dynamics_torch(s, tau, self.rls_parameters)
    ...
    def solve(self, state, ref, tau_prev):
        ...
        for n in range(N):
            tau = U[:, n]
            J = J + self._stage_cost(s, tau, prev, ref)
            s = self._step(s, tau, dt)
            prev = tau

        e = s - ref

        terminal_angle_difference = s[:, 2] - ref[2]

        terminal_theta_error = torch.atan2(
            torch.sin(terminal_angle_difference),
            torch.cos(terminal_angle_difference),
        )

        terminal_speed_error = s[:, 1] - ref[1]
        terminal_omega_error = s[:, 3] - ref[3]

        J = J + (
            cfg.q_terminal_theta * terminal_theta_error**2
            #+ cfg.q_v * terminal_speed_error**2
            #+ cfg.q_terminal_omega * terminal_omega_error**2
        )
        J = torch.nan_to_num(J, nan=1e12, posinf=1e12, neginf=1e12)

        rho = J.min()
        w = torch.exp(-(J - rho) / lam)
        du = (w.unsqueeze(1) * eps).sum(0) / (w.sum() + 1e-8)
